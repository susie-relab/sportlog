...
import { ExerciseType, EXERCISE_TYPE_LABELS, EXERCISE_TYPE_COLORS } from '@/types';
...
const ACTIVITY_TYPES: { value: ActivityFilter; label: string; color: string }[] = [
  ...
  { value: 'hiit', label: 'HIIT', color: EXERCISE_TYPE_COLORS['hiit'] },
  { value: 'sport', label: 'Sport', color: EXERCISE_TYPE_COLORS['sport'] },
  { value: 'stretch', label: 'Stretch', color: EXERCISE_TYPE_COLORS['stretch'] },
  { value: 'solo_fitness', label: 'Solo Fitness', color: EXERCISE_TYPE_COLORS['solo_fitness'] },
];
...
export default function TrainingPlanPage() {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      target_runs: goal.target_runs ?? null,
      target_distance_km: goal.target_distance_km ?? null,
      target_minutes: goal.target_minutes ?? null,
      target_activities: goal.target_activities ?? null,
    };
    const { data, error } = await supabase
      .from('goals')
      .upsert(payload, { onConflict: 'user_id,period,activity_type' })
      .select()
      .single();
    setSaving(false);
    if (!error && data) {
        ...
        {(activeType === 'all' || activeType === 'run') && (
          <div>
            ...
            <input type="number" className="input" placeholder="e.g. 4" min="0"
              value={goal.target_runs ?? ''} onChange={e => update('target_runs', e.target.value)} />
          </div>
        )}
        <div>
          <label className="label">Target Distance (km)</label>
          <input type="number" className="input" placeholder="e.g. 50" min="0" step="0.1"
            value={goal.target_distance_km ?? ''} onChange={e => update('target_distance_km', e.target.value)} />
        </div>
        <div>
          <label className="label">Target Time (minutes)</label>
          <input type="number" className="input" placeholder="e.g. 300" min="0"
            value={goal.target_minutes ?? ''} onChange={e => update('target_minutes', e.target.value)} />
          {goal.target_minutes ? (
            <p className="text-xs text-[#475569] mt-1">= {Math.floor(goal.target_minutes / 60)}h {goal.target_minutes % 60}m</p>
          ) : null}
        </div>
        <div>
          <label className="label">Target Activities</label>
          <input type="number" className="input" placeholder="e.g. 6" min="0"
            value={goal.target_activities ?? ''} onChange={e => update('target_activities', e.target.value)} />
        </div>

        {saved && (
          <div className="p-3 rounded-lg bg-green-900/40 border border-green-700 text-green-300 text-sm">✅ Goals saved!</div>
        )}
