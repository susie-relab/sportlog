...
export default function CustomPlanBuilder({ existing, onSaved, onCancel, activeByDefault = true }: Props) {
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      start_date: startDate, name: name.trim() || 'Sport Plan', active: existing ? existing.active : activeByDefault, plan_data: preview,
        ...
        {subs && (
          <>
            ...
            <div className="bg-[#0F172A] -mx-5 px-5 pt-3 pb-4">
              ...
              <div className="grid grid-cols-3 gap-1.5">
                <button onClick={() => { setSelSub(''); }} className={`py-1.5 px-2 rounded-lg text-[11px] font-medium border transition-all ${!selSub ? 'bg-blue-600 border-blue-600 text-white' : 'border-[#334155] text-[#94A3B8] hover:border-[#475569]'}`}>Generic</button>
